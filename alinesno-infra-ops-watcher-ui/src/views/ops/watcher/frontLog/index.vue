<template>
  <div class="app-container">
    ...
    <el-dialog :title="title" v-model="open" width="900px" append-to-body>
      <el-form :model="form" :rules="rules" ref="ProjectRef" label-width="80px">
        <el-row>
          <el-col :span="24">
            <el-form-item  label="应用名称" prop="applicationName">
...
import {
  ...
  updateProject,
  addProject,
} from "@/api/ops/watcher/project";
import {reactive} from "vue";

const router = useRouter();
const {proxy} = getCurrentInstance();
// const { sys_normal_disable, sys_Project_sex } = proxy.useDict("sys_normal_disable", "sys_Project_sex");
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除应用编号为"' + applicationIds + '"的数据项？').then(function () {
    return delProject(applicationIds);
  }).then(() => {
    getList();
...
function handleUpdate(row) {
  ...
  getProject(applicationId).then(response => {
