  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :span="24" :xs="24">
        ...
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button
                ...
                :disabled="single"
                @click="handleUpdate"
                v-hasPermi="['system:NginxAccessLog:edit']"
            >修改
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
                ...
                :disabled="multiple"
                @click="handleDelete"
                v-hasPermi="['system:NginxAccessLog:remove']"
            >删除
            </el-button>
          </el-col>
        ...
        <el-table v-loading="loading" :data="NginxAccessLogList" @selection-change="handleSelectionChange">
            ...
            <template #default="scope">
              <el-tooltip content="修改" placement="top" v-if="scope.row.applicationId !== 1">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                           v-hasPermi="['system:NginxAccessLog:edit']"></el-button>
              </el-tooltip>
              <el-tooltip content="删除" placement="top" v-if="scope.row.applicationId !== 1">
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                           v-hasPermi="['system:NginxAccessLog:remove']"></el-button>
              </el-tooltip>
    ...
    <el-dialog :title="title" v-model="open" width="900px" append-to-body>
      <el-form :model="form" :rules="rules" ref="NginxAccessLogRef" label-width="80px">
        <el-row>
          <el-col :span="24">
            <el-form-item  label="应用名称" prop="applicationName">
...
<script setup name="NginxAccessLog">
import {getToken} from "@/utils/auth";
import {
  listNginxAccessLog,
  delNginxAccessLog,
  getNginxAccessLog,
  updateNginxAccessLog,
  addNginxAccessLog,
} from "@/api/ops/watcher/nginxAccessLog";
import {reactive} from "vue";

const router = useRouter();
const {proxy} = getCurrentInstance();
// const { sys_normal_disable, sys_NginxAccessLog_sex } = proxy.useDict("sys_normal_disable", "sys_NginxAccessLog_sex");

const NginxAccessLogList = ref([]);
...
const upload = reactive({
  ...
  // 上传的地址
  url: import.meta.env.VITE_APP_BASE_API + "/system/NginxAccessLog/importData"
});
// 列显隐信息
const columns = ref([
...
function getList() {
  loading.value = true;
  listNginxAccessLog(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
    loading.value = false;
    NginxAccessLogList.value = res.rows;
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除应用编号为"' + applicationIds + '"的数据项？').then(function () {
    return delNginxAccessLog(applicationIds);
  }).then(() => {
    getList();
...
function reset() {
  ...
  proxy.resetForm("NginxAccessLogRef");
};
...
function handleUpdate(row) {
  ...
  getNginxAccessLog(applicationId).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["NginxAccessLogRef"].validate(valid => {
    if (valid) {
      if (form.value.applicationId != undefined) {
        updateNginxAccessLog(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addNginxAccessLog(form.value).then(response => {
