  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :span="24" :xs="24">
        ...
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button
                ...
                :disabled="single"
                @click="handleUpdate"
                v-hasPermi="['system:ContainerLog:edit']"
            >修改
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
                ...
                :disabled="multiple"
                @click="handleDelete"
                v-hasPermi="['system:ContainerLog:remove']"
            >删除
            </el-button>
          </el-col>
        ...
        <el-table v-loading="loading" :data="ContainerLogList" @selection-change="handleSelectionChange">
            ...
            <template #default="scope">
              <el-tooltip content="修改" placement="top" v-if="scope.row.applicationId !== 1">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                           v-hasPermi="['system:ContainerLog:edit']"></el-button>
              </el-tooltip>
              <el-tooltip content="删除" placement="top" v-if="scope.row.applicationId !== 1">
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                           v-hasPermi="['system:ContainerLog:remove']"></el-button>
              </el-tooltip>
    ...
    <el-dialog :title="title" v-model="open" width="900px" append-to-body>
      <el-form :model="form" :rules="rules" ref="ContainerLogRef" label-width="80px">
        <el-row>
          <el-col :span="24">
            <el-form-item  label="应用名称" prop="applicationName">
...
<script setup name="ContainerLog">
...
import {
  ...
  updateContainerLog,
  addContainerLog,
} from "@/api/ops/watcher/containerLog";
import {reactive} from "vue";

const router = useRouter();
const {proxy} = getCurrentInstance();
// const { sys_normal_disable, sys_ContainerLog_sex } = proxy.useDict("sys_normal_disable", "sys_ContainerLog_sex");

const ContainerLogList = ref([]);
...
const upload = reactive({
  ...
  // 上传的地址
  url: import.meta.env.VITE_APP_BASE_API + "/system/ContainerLog/importData"
});
// 列显隐信息
const columns = ref([
...
function getList() {
  loading.value = true;
  listContainerLog(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
    loading.value = false;
    ContainerLogList.value = res.rows;
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除应用编号为"' + applicationIds + '"的数据项？').then(function () {
    return delContainerLog(applicationIds);
  }).then(() => {
    getList();
...
function reset() {
  ...
  proxy.resetForm("ContainerLogRef");
};
...
function handleUpdate(row) {
  ...
  getContainerLog(applicationId).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["ContainerLogRef"].validate(valid => {
    if (valid) {
      if (form.value.applicationId != undefined) {
        updateContainerLog(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addContainerLog(form.value).then(response => {
