...
@RequestMapping("/api/infra/ops/watcher/rule")
public class RuleController extends BaseController<RuleEntity, IRuleService> {
    ...
    public TableDataInfo datatables(HttpServletRequest request, Model model, DatatablesPageBean page) {
        ...
        long countGitRepository = service.count(new LambdaQueryWrapper<RuleEntity>().eq(RuleEntity::getOperatorId , userId));

        // 初始化用户仓库
        if (countGitRepository == 0) {
            service.initRule(CurrentAccountJwt.getUserId());
        }
